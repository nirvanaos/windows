...
const COFF::PE32Header* COFF::pe32_header () const noexcept
{
	...
	if (hdr->SizeOfOptionalHeader >= offsetof (PE32Header, DataDirectory)) {
		const PE32Header* pehdr = (const PE32Header*)(hdr + 1);
		if (pehdr->Magic == pe_bliss::pe_win::image_nt_optional_hdr32_magic
			|| pehdr->Magic == pe_bliss::pe_win::image_nt_optional_hdr64_magic)
			return pehdr;
	}
	return nullptr;
}
...
{
	const char* sn = (const char*)s.Name;
	char c;
	while ((c = *name) && *sn == c) {
		++sn;
		++name;
	...
	if (c)
		return false;
	if (sn - (const char*)s.Name < SECTION_NAME_SIZE)
		return !*sn;
	else
		return true;
}
